function ImageProcessor() {
	...
	this.resize = function(imgData) {
		...
		return this.loadImage(imgData)
			.then(function(imgData){
				...
				//debugger;
				if(imgData.ratio.width) {
					imgData.ratio = imgData.ratio.width / imgData.img.width;
				}
				else if (imgData.ratio.height){
					imgData.ratio = imgData.ratio.height / imgData.img.height;
				}
	...
	this.toJpegBlob = function(filename, ratio, quality) {
		//debugger;
		ratio = (ratio && ((!typeof(ratio) == 'number') || ratio.height || ratio.width) ? ratio : 1);
		var imgData = new Object({filename:filename, ratio: ratio, quality:parseFloat(quality) || 0.3});
