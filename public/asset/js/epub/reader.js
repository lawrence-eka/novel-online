...
(function(root, $) {
	...
	var ePubReader = root.ePubReader = function(path, options, renderTo) {
		return new EPUBJS.Reader(path, options, renderTo);
	};
...
EPUBJS.Reader = function(bookPath, _options, _renderTo) {
	_renderTo = _renderTo || "viewer";
	var reader = this;
	var book;
	var plugin;
	//var $viewer = $("#viewer");
	var $viewer = $(typeof _renderTo == "string" ? document.getElementById(_renderTo) : EPUBJS.core.isElement(_renderTo) ? _renderTo : null);
	...
	book.renderTo(_renderTo);//("viewer");
