yalla.framework.addComponent("/dist/user/userProfile", (function() {
  ...
  function onRegister() {
    ...
    if ((profile.isPublisher ? true : false) != form.elements.isPublisher.checked) {
      profile.isPublisher = (form.elements.isPublisher.checked);
    } else {
      delete profile.isPublisher;
      if (profile.needApproval) delete profile.needApproval.isPublisher;
    }

    //	    if(profile.needApproval && !profile.needApproval.hasOwnProperty('isAdmin') && !profile.needApproval.hasOwnProperty('isTravelAgent')) {
    if (profile.needApproval && !profile.needApproval.hasOwnProperty('isPublisher')) {
        ...
        (function(domNode) {
          ...
          function asyncFunc_1(data) {
            ...
            yalla.framework.registerRef("alert", IncrementalDOM.currentElement(), function() {
              ...
              _context["entry"].render(typeof arguments[1] === "object" ? _merge(arguments[1], _params) : _params, function(slotName, slotProps) {});
              if (!gotNeedApproval(data, 'isPublisher')) {
                _elementOpenStart("div", "");
                _elementOpenEnd("div");
                var _params = {
                  "type": "checkbox",
              ...
              if (gotNeedApproval(data, 'isPublisher')) {
                ...
                var _params = {
                  ...
                  "prompt": "Publisher*",
                  "name": "isPublisher",
                  "checked": data.needApproval.isPublisher,
                  "onclick": function(event) {
                    var self = {
                      target: event.target
                    };
                ...
                var _params = {
                  "type": "label",
                  "prompt": ('*' + (data.needApproval.isPublisher ? 'Request for' : 'Revokal of') + ' Publisher Access under review')
                };
                _context["entry"].render(typeof arguments[1] === "object" ? _merge(arguments[1], _params) : _params, function(slotName, slotProps) {});
                _elementClose("div");
              }
